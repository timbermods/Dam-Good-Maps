...
- **Project file** (`.damgoodmaps.json`, gzip-compressed): the `MapDocument` with its spec,
  ...
  without 3D forms store an empty list. A document opens exactly even after the generator has changed: it shows
...
| D4 | Terrain stays at 16 or below in generated maps and editor tools. Imported maps with terrain up to 22 are preserved. | 16 is the in-game editor's limit; 17–22 is untested (§18 E1). | Audit default, accepted by Kyler; **amended by D123** (P3D-5) |
