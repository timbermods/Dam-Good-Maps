...
- The player picks one of *fun / too easy / boring terrain / frustrating / broken* and can add a
  ...
  (1–5), with an optional note. From D137 no rating refits the score: `fit-score.ts` is not used.
...
- The ratings are read as feedback. They don't tune the score, which keeps its default weights
  (D137); the in-site feedback of D137 steers each theme's priors and the Variety setting.
...
- **Height.** Only at high Verticality (70 and above) may generated terrain go above 16, up to the
  ...
  16, like the official maps and the in-game editor. Before M9a offers heights above 16, a DGM Probe
  batch confirms that such maps load and keep their terrain, water and objects (§18 E1; the Probe's
  T6).
