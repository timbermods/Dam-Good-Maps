...
| D147 | **Map look 2: high fidelity** (ROADMAP), after the Map quality checkpoint and before the Frame pass: a graphics quality setting (High, chosen automatically on capable GPUs; Standard, today's clean look; Light, the software look). High adds a real water shader (ripples catching the light, clear shallows, colour by depth, reflections by angle, shore foam; Kyler's direction, 2026-09-25: fewer, subtler sparkle flecks than the clean look, and more depth and transparency), real-time soft shadows, ambient occlusion and a warm colour grade, higher-resolution procedural grass, earth and cobbles, softened block edges and grass lips, full-resolution rendering with anti-aliasing, and more detailed tree, bush and ruin models of our own. Our own art only; never game assets. Judged by eye against Kyler's reference screenshots from captures; speed is information only, but no mode may feel sluggish on the machines it's chosen for. Released as `map-look-2-done`. | Kyler wants the map to look much closer to the game where the hardware allows. | Kyler's decision, 2026-09-25 |
...
| D160 | **"Pick a place"** (moved by Kyler on 2026-09-25 from after M11 to right after Live editing, alongside M9; it doesn't touch the generator): choose any spot on a world map (or paste coordinates, for example from Google Earth), pick size and scale, and get a playable map built from open elevation data with attribution, through the same conversion pipeline. Never Google's own data. If a place converts poorly, say so plainly and suggest a larger area or a different scale. | Kyler: real places on demand. | Kyler's decision, 2026-09-25 |
