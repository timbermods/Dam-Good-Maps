...
| D203 | **Quake, and the forces group** (a new tool). Quake gets its own top-bar button next to Carve and Craterize, in a visually distinct "forces" group on the bar. It splits the land along a drawn fault line: Lift or Slide; Power; Sheer or Stepped scarp; Try another (including a natural tilt); objects ride with the land; it refuses a fault through the start; it never adds water; one undo step; Esc reverts. Codex is prototyping it on `investigation/quake`, building on Carve and Craterize: when its PR is open it is held until Kyler says it's ready. When the forces are adopted into the editor, all three (Carve, Craterize, Quake) are built on one shared forces core. | Kyler: the forces of nature are a family. | Kyler's decision, 2026-09-26 |
