...
| Berries near start | 20 – 100 | by difficulty (Easy 20, Normal 48, Hard 60): the generation target; §5.6 gives the lower minimum that validation enforces (from M8: Minimum starting bushes, D85) | Living bushes within 20 tiles of the start, in 2–3 patches beside water. |
...
**Start requirements** (Kyler, 2026-09-24; D85; built at the start of M8). The same on every
difficulty. They are the only start rules that reject a map (§11.4):

| Requirement | Rule | Setting |
|---|---|---|
| Water without stairs | Clean water (depth ≥ 0.3, contamination < 0.05) touches a shore tile at the start's own level, and that shore tile is walkable from the start without any slope: the same level all the way. Rivers, lakes and ponds all count. A pump on that shore reaches the water surface (0–2 levels below). No distance limit. | — |
| Starting trees | Living trees within 20 tiles' walk of the start (slopes allowed), counted across any number of groves, at least the setting. | **Minimum starting trees**, 0–400, default 50 (`st`) |
| Starting bushes | Living berry bushes within 20 tiles' walk of the start (slopes allowed), counted across any number of patches, at least the setting. | **Minimum starting bushes**, 0–200, default 20 (`sb`) |

"Living" means the plant survives at steady state. The two minimums are the Advanced start rules
controls for trees and living bushes within 20, renamed; they keep their ranges and share-link
keys, so old links still decode. Their defaults are 50 and 20 on every difficulty, and changing
Designed for doesn't reset them. The generator never aims below either minimum: when a player
raises one above the difficulty's target (below), the target rises with it. Imported maps, which
have no settings, use the defaults.
...
an advisory warning when a map misses one, but they no longer reject a map (D85). Until then they
reject maps as built in M2–M7, with living bushes within 20 at 20 / 40 / 40 and trees counted by
straight distance, dead ones included.
...
- **Budget** (revised from 1.5 s / 0.4 s, which the measurement does not support for a cold
  ...
  - **M6 measurement** (generator 0.4.0, the same benchmark with `--theme`, seeds 1–10): River
    ...
    ticks), over the budget (D68). CI's gate runs River Valley.
...
| Id | Rule | From M8 |
|---|---|---|
| `start.dry` | No water within Chebyshev 2 of the start centre after settling. | rejects |
| `start.water` | Requirement 1, water without stairs: clean water (depth ≥ 0.3, contamination < 0.05) touches a shore tile at the start's own level, walkable from the start without any slope, and a pump on that shore reaches the surface (0–2 levels below). No distance limit. Before M8: that water within the difficulty distance (10 / 16 / 22). | rejects |
| `start.reach_water` | Before M8: that water borders land walkable from the start. From M8 it is part of `start.water`. | — |
| `start.water_distance` (new) | The start's pumpable clean water within the difficulty distance (10 / 16 / 22). | advisory |
| `start.wood` | Requirement 2: living trees within 20 tiles' walk (slopes allowed) ≥ Minimum starting trees (default 50). Before M8: trees within 20 tiles and reachable ≥ 80 / 50 / 40. | rejects |
| `start.food` | Requirement 3: living berry bushes within 20 tiles' walk (slopes allowed) ≥ Minimum starting bushes (default 20). Before M8: within 20 tiles and on or beside reachable land ≥ 20 / 40 / 40. | rejects |
| `start.badwater` | No badwater water or contaminated soil within the badwater distance. | advisory |
...
- **3D (lazy):** three.js orbit view of the terrain columns, water surfaces as translucent quads at
  ...
  switch, loaded on demand. Map look (after M8, D86) colours the 3D view's ground by moisture, as
  the game does, with a toggle back to height colours; the 2D preview keeps its height colours.
