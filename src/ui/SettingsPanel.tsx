  ...
  return (
    <div class="settings-panel">
      ...
      <Section title="Advanced: start rules">
        ...
        <Num id="rule-water" label="Clean water within (tiles)" value={s.start.rules.waterWithin} min={4} max={40} band={`${band("waterWithin", spec)} Default ${d.waterWithin}.`} onChange={(v) => set((c) => (c.start.rules.waterWithin = v))} />
        <Num id="rule-trees" label="Trees within 20 tiles" value={s.start.rules.treesWithin20} min={0} max={400} band={`${band("treesWithin20", spec)} Default ${d.treesWithin20}.`} onChange={(v) => set((c) => (c.start.rules.treesWithin20 = v))} />
        <Num id="rule-bushes" label="Living bushes within 20 tiles" value={s.start.rules.bushesWithin20} min={0} max={200} band={`${band("bushesWithin20", spec)} Default ${d.bushesWithin20}.`} onChange={(v) => set((c) => (c.start.rules.bushesWithin20 = v))} />
