// The field (docs/m9-design.md §4): the land's processes, before it is snapped to levels.
// - Uplift: a regional slope (toward an edge, or toward a bowl), a slow regional field of broad
//   highs and lows beside it (an independent spatial control, playbook #1), domain-warped fractal
//   noise, and the genome's parts (ridges, troughs, basins, calderas, mesas, mesa fields,
//   escarpments that die out along their length, cones, knolls, spirals).
// - Caprock: hard rock in patches of the upper stratum, that erosion and weathering cut round.
// - Stream-power erosion (Braun and Willett) with that hardness, and diffusion.
// - Weathering: soft high ground wastes down toward the ground round it, so caprock stands up as
//   stacks, buttes and mesas, and a mesa with a soft middle keeps a rim round a hollow.
...
function addPart(U: Float64Array, p: Part, seed: number, W: number, H: number, k: number): void {
  ...
  switch (p.kind) {
    ...
    case "escarpment": {
      // a long wobbling cliff line that dies out along its length (`size` tiles long)
      const [ux, uy] = unit(p.turn);
...
export function caprock(U: Float64Array, g: Genome, seed: number, W: number, H: number): { hard: Float64Array; stratum: Float64Array } {
  const N = W * H;
...
/** Stream-power incision (implicit, Braun and Willett 2013, m = 0.5, n = 1) and diffusion, with hardness: hard rock resists incision, and
...
export function makeField(g: Genome, seed: number, W: number, H: number): Field {
  const U = uplift(g, seed, W, H);
