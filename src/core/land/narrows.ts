// The natural-narrows builder (design version 2, D111; #63: no Dam site tool, kept as an internal
// operation for M12's Claude): two hillside spurs close in on a river from its two banks, each a
// tongue of ground of uneven thickness that falls 1–2 levels from its root on the valley side to
// its tip by the channel, the two drawn apart (lengths, bends, heights, thicknesses) and offset
// along the river, so together they never read as one straight band. The channel keeps its gap; a
// dam across the gap is the player's to build. The generator never calls it: it is an edit Claude
// asks for.
//
// The plan refuses a result the dam-wall check (analysis/ridge.ts) flags, and says why; it then
// tries once more with the spurs drawn further apart before giving up. Ported from
// investigation/generative/v2/narrows.ts unchanged.
