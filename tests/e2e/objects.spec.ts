// ROADMAP M7 through the page: a forest drawn on the map shows where its trees live before it is
// placed; an object shows its footprint under the pointer, green where the game keeps it and red
...
test("a forest shows where its trees live before it is placed", async ({ page }) => {
  const errors: string[] = [];
  ...
  await expect(card).toContainText(/alive where the soil stays moist/);
