...
import { INDEX, PLACES_DIR, PLACES_HAVE_EDGE_WALLS, placeData, sha256 } from "./placesCommon";
...
describe.skipIf(!PY)("both validators agree on the sample (prototype/validate.py)", () => {
  it("every check has the same verdict, and every map passes both but for the edge wall, which both flag", () => {
    ...
    for (const [k, e] of SAMPLE.entries()) {
      ...
      expect(rep!.passed, e.id).toBe(!PLACES_HAVE_EDGE_WALLS);
      expect(rep!.checks.filter((c) => !c.ok && !c.na && !c.approx && !(c as { advisory?: boolean }).advisory).map((c) => c.id), e.id).toEqual(PLACES_HAVE_EDGE_WALLS ? ["terrain.edge_wall"] : []);
      const b = built(e.id);
    ...
    expect(r.status).toBe(PLACES_HAVE_EDGE_WALLS ? 1 : 0);
  });
});
