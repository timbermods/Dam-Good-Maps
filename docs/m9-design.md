...
Canyon 14; River Valley and Highlands 13; Lake Basin, Delta and Islands 12; 13 over all six
(official 13, workshop 14; version 1 7–11). Levels in use: 15 (official and workshop 16). Tallest
fall median 5.1 levels (official 3.9, workshop 5.7). Flat share 0.41 (official 0.52, workshop 0.44); cliff
share 0.15 (official 0.16, workshop 0.11). The highest ground stands at 15 on the median map (14 at
p10, 16 at p90); none goes above 16 at the default.
...
3. **Many structural realizations.** No-clone and no-archetype run within each intention, over the
   ...
   nearest pair 0.38 or more apart). The largest cluster holds 6–13% for most intentions. It holds
   17–20% for the waterfall shield and the crater, but on only 25–29 maps.

**Kyler's three new intentions, drawn on every map** (seeds 1–30 of every theme, 180 maps each; a
steering test, REPORT-v2 §4): the snaking river emerges on 66%, the crater on 28% and the
waterfall into a round lake on 17%. None has clones (nearest pairs 0.43, 0.49 and 0.47 apart). The
largest cluster holds 6.7% of the snaking rivers and 5.9% of the craters, but 23% (7 of 30) of the
waterfalls: tilted-plane maps with no islands and little water, where the nudge's tall scarp makes
one big step. A shorter, tapered scarp is M9b's fix.

The mix varies by map: of 1,200 maps, 293 have none, 614 one and 293 two. Each brief names its
...
- no built dam walls on any map; batches 100% final, 68.5% on the first attempt;
- M2a whole maps: the largest cluster holds 6–14.5% of a theme (version 1: 11.5–68%). Canyon's
  water prior was drawn wider (more lakes, spring lakes and river islands) to bring its cluster
  from 19.5% to 11% once the new intentions were in the set;
- M1 no clones: every map's nearest other seed is 0.36–0.45 away at least and 0.48–0.52 at the
  median by theme (version 1: 0.33–0.38 and 0.42–0.48; targets 0.25 and 0.40);
- M3 openings: the largest cluster holds 1.5–2% of a theme and the spread is 0.76–0.92 (a
  workshop pair is 1; version 1 0.68–0.84); M3d strategy axes: 185–196 joint signatures in 200 maps
  per theme, the largest shared by 1–1.5%;
- M4 no approximation: River Valley 8.5%, Canyon 0.5%, Highlands 3.5%, Delta 12.5%, Islands 15%,
  Lake Basin 12.5% of maps below the floor (a target of 10% at most; information, D128). The close
  pairs are close on islands (none), one-level steps, scrap, cliffs, relief and flat land in River
  Valley and Highlands. In Delta no single feature stands out: basins, scrap, one-level steps,
  bushes and dam sites all agree a little (REPORT-v2 §3.4);
- M5 a good natural dam site within 40 tiles of the start: 49.5–76.5% by theme, 64% over all six
  (official 36%, workshop 46%; the band "comparable to the official maps" is 26–56%): the lakes'
  outlets make short dams, so four themes sit above the band;
...
Format 3's terrain (field and base, heights as base64 as DESIGN §2.2 has it) takes 6.5 KB gzipped
at 128² and 19.8 KB at 256²: more than version 1's run-length field alone (2.3 and 6.5 KB), beside
...
1. **The field once**: a failed attempt whose land was fine (no start, no storage, too little
   wood, no clean water) is planned again on the same field before a new genome is drawn. 87% of
...
2. **One settle**: the hollow is planned before the settle from where the start will likely be,
   ...
   One settle for 80% of maps, two for 20%, three for 0.2% (version 1: two to four an attempt).
...
200 seeds per theme at 128², 30 at 96², 192² and 256², Normal (REPORT-v2 §2). Final: 100% in
every theme and size, and at Variety 100; at Verticality 85, 100% but Canyon's 99% (one seed out of
100, after 12 attempts). First attempt: 68.5% at 128² over all six (Canyon lowest, 53.5%). Most failed attempts had no storage
near the start, no place for a start, or too little wood within reach; each is planned again on the
same field first.
...
**Risks:**
- *No approximation in the water-rich themes*: Delta, Islands and Lake Basin sit at 12.5–15%. More
  water drew Islands and Lake Basin toward the workshop's middle. M9b can push their water and
  their one-level steps apart. Information (D128).
- *Intentions that rarely emerge*: the long view, meeting waters and farmland past the gorge emerge
  on 21–24% of draws, and Kyler's crater and waterfall into a round lake on 24% and 19.5%. If M9b's
