...
export interface Plant { tile:number; species:string; initialDead:boolean; dead:boolean; dry:number; flood:number; bad:number; dryLimit:number; badLimit:number; diedAt:number|null; cause:string|null; }
...
export class CycleModel {
  ...
  constructor(readonly built: BuildResult, readonly seed=1729, readonly soilTicks=SOIL_TICKS) {
    ...
    if (built.settle.out) this.sim.out.set(built.settle.out);
    this.barrier=moistureBarrier(built.W,built.H,objects);
    this.M=built.moisture.slice(); this.SC=built.soilContamination.slice(); this.candidates=this.SC.slice();
    ...
    if(this.sources.length!==this.sim.emitters.length) throw new Error('Emitter/object order mismatch');
  ...
  tick(phase: Phase, phaseTick: number): void {
    ...
    this.sim.run(1); this.elapsedTicks++; this.soilElapsed++;
    if(this.soilElapsed===this.soilTicks) { this.updateLife(this.soilElapsed); this.soilElapsed=0; }
  }
  updateLife(ticks:number): void {
    ...
    for(let i=0;i<s.N;i++) {
      ...
      if(this.M[i]<.01)this.M[i]=0;
      if(this.SC[i]<.001)this.SC[i]=0;
      if(this.barrier?.[i]) {this.M[i]=0;this.SC[i]=0;this.candidates[i]=0;}
    }
    ...
    for(const p of this.plants) {
      ...
      if(cause) {p.dead=true;p.cause=cause;p.diedAt=this.elapsedTicks/TICKS_PER_DAY;}
    }
  }
  run(phase:Phase,onDay?:(day:number,model:CycleModel)=>void):void {
    ...
    for(let t=0;t<ticks;t++) {
      this.tick(phase,t);
      if((t+1)%TICKS_PER_DAY===0||t+1===ticks) onDay?.((t+1)/TICKS_PER_DAY,this);
    }
  }
}
