...
for (const theme of THEMES)
    for (const [size, seed] of [[96, 1], [128, 2]]) {
        ...
        const phases = all.slice(0, firstBad + 1);
        phases.push({ weather: 'normal', days: 5, cycle: phases.at(-1)!.cycle + 1, occurrence: 6, previous: 'badtide' });
        let elapsed = 0;
        const days: any[] = [], frames: any[] = [];
        const t = performance.now();
        for (const phase of phases) {
            model.run(phase, (day, m) => { days.push({ ...measure.sample(elapsed + day), phase: phase.weather, phaseDay: day, cycle: phase.cycle }); frames.push(frame(m, measure.initialWet)); });
            elapsed += phase.days;
        }
