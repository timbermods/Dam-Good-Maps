...
export interface Phase { weather: Weather; days: number; cycle: number; occurrence: number; previous?: Weather; next?: Weather; }
export const MODES = {
...
export function badChance(previous: Weather | undefined, streak: number): number {
  ...
  let likelihood = 1;
  for (let i=0;i<streak+1;i++) likelihood *= p;
...
export function sourceScale(strength: number, phase: Phase, day: number): number {
  if (phase.weather==='drought') return 0;
  if (phase.weather==='badtide' || strength<=0) return 1;
  const transition = strength/(460.8*.0058);
  const curve = (p: number) => p*(.85*p+.15);
  if (phase.next==='drought' && day>=phase.days-transition) return 1-curve(Math.min(1,(day-phase.days+transition)/transition));
  if (phase.previous==='drought' && day<transition) return curve(Math.max(0,day/transition));
  return 1;
}
