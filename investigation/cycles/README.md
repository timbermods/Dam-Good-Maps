...
The 12 maps cover all six themes. Each has daily pictures of normal weather, first droughts on Easy, Normal and Hard, a first badtide, and a later Hard drought. **First five cycles** follows one map continuously through its first badtide. The other stretches start from the same settled map, for comparison. Recovery follows each hazard.
...
This is an estimate of an untouched map. It does not subtract drinking water or add new seedlings. Daily event times mean “by this day.” Soil timing still needs an in-game check.
...
- [Calibration](CALIBRATION.md): one evening of in-game checks.
- [Integration proposals](INTEGRATION.md): preview, editor, cards and M9.
