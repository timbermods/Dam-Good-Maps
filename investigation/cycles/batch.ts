...
for(const p of ['results/maps','viewer/data','generated'])mkdirSync(`${dir}/${p}`,{recursive:true});
...
export function cases():{id:string;label:string;phases:Phase[]}[] {
...
for(const size of sizes)for(const theme of themes)for(const seed of seeds) {
  const id=`${theme}-${size}-${seed}`,path=`${dir}/results/maps/${id}.json`;
  if(existsSync(path)&&!process.argv.includes('--force')){console.log(`cached ${id}`);continue;}
  const start=performance.now();
  const r=generate(makeSpec({seed,theme,size:{x:size,y:size}})),generationMs=performance.now()-start;
  if(!r.report.passed){writeFileSync(path,JSON.stringify({id,theme,size,seed,passed:false,failures:r.failures},null,2));console.log(`FAILED ${id}`);continue;}
  // Same generate/makeSpec path as tools/gen.ts; unlike that CLI this accepts every theme.
  if(study==='gallery')writeFileSync(`${dir}/generated/${id}.timber`,r.bytes);
  const sha256=createHash('sha256').update(r.bytes).digest('hex');
  let t=performance.now();const canonical=temperateSettle(r.built.waterModel);const settleMs=performance.now()-t;
  ...
  if(parity.depth!==0||parity.contamination!==0)throw new Error(`${id}: settle mismatch`);
  const scenarios:any[]=[];
  ...
  for(const spec of cases()) {
    t=performance.now();const model=new CycleModel(r.built),measure=new Measures(model,r.spec.settings.start.rules.waterWithin);
    regions=measure.regions;baseline=measure.sample(0);
    if(!visual.plants.length)visual.plants=model.plants.map(p=>({tile:p.tile,species:p.species,dead:p.initialDead}));
    let elapsed=0;const days:any[]=[];const images:any[]=[];
    for(const phase of spec.phases) {
      model.run(phase,(day,m)=>{
        const row={...measure.sample(elapsed+day),phase:phase.weather,phaseDay:day,cycle:phase.cycle};
        days.push(row);if(gallery)images.push(frame(m,measure.initialWet));
      });elapsed+=phase.days;
    }
    ...
    scenarios.push(record);if(gallery)visual.scenarios.push({...record,frames:images});
  }
  t=performance.now();const comparisonDays=[9,cases().find(c=>c.id==='late-hard')!.phases[1].days].sort((a,b)=>a-b);
  const sim=new WaterSim(cloneModel(r.built.waterModel),{depth:r.built.water,contamination:r.built.contamination});let ran=0;
  const droughtComparison=comparisonDays.map(days=>{
    sim.run((days-ran)*768,0);ran=days;const analytic=droughtStorage(r.built.waterModel,r.built.water,days),a=sum(analytic),v=sim.volume();
  ...
  if(gallery){visual.regions=regions;visual.sha256=sha256;writeFileSync(`${dir}/viewer/data/${id}.json.gz`,gzipSync(JSON.stringify(visual),{level:9}));}
