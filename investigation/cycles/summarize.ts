...
const signature = (m: any) => {
    ...
    const hazardStart = long.days.find((x: any) => x.phase !== 'normal').day;
    const startDays = long.firstWaterLost === null ? 26 : Math.max(0, long.firstWaterLost - hazardStart);
...
const timings = [96, 128, 256].map(size => {
    ...
    return { size, n: rows.length, generationMedian: q(rows.map(m => m.timings.generationMs)) / 1000,
        ...
        cpuDayMedian: q(rows.map(m => m.scenarios.reduce((s: number, x: any) => s + x.cpuMs, 0) / m.scenarios.reduce((s: number, x: any) => s + x.phases.reduce((a: number, p: any) => a + p.days, 0), 0))) / 1000 };
});
...
const worstDepth = comparisons.reduce((a, c) => c.maxDepthError > a.maxDepthError ? c : a, comparisons[0]);
const generatedToleranceFailures = comparisons.filter(c => c.initialVolumeError > .05 || c.maxDepthError > .1);
// Settle stability: the first temperate period up to the day before the sources start to slow. The
// drought probe's lead is the number of days the slowdown can take on this map.
function temperateDrift(m: any): number {
    const normal = m.scenarios.find((s: any) => s.id === 'normal'), lead = m.scenarios.find((s: any) => s.id === 'first-normal').days.find((d: any) => d.phase !== 'normal').day;
    const day = normal.days.at(-1).day - lead, row = normal.days.find((d: any) => d.day === day);
    return Math.abs(row.volume / normal.days[0].volume - 1);
}
const summary = { complete: !partial, maps: maps.length, passed: good.length, failed: maps.filter(m => !m.passed).map(m => m.id), survey: survey.length,
    ...
    drought: { comparisons: comparisons.length, withinFivePercent: comparisons.length - failures.length, failures, maxInitialVolumeError: baselineMax, maxDepthError: depthMax, worstDepth: { id: worstDepth.id, days: worstDepth.days }, generatedToleranceFailures },
    normalDriftMax: Math.max(...good.map(temperateDrift)), normalEndMax: Math.max(...good.map(m => Math.abs(m.scenarios.find((s: any) => s.id === 'normal').finalRetention - 1))) };
if (partial) {
