...
reference.Dold.set(reference.D);reference.out.set(r.built.settle.out!);
normal.run({weather:'normal',days:1,cycle:1,occurrence:1});reference.run(768);
assert.equal(maxDiff(normal.sim.D,reference.D),0);assert.equal(maxDiff(normal.sim.C,reference.C),0);
const measure=new Measures(new CycleModel(r.built),r.spec.settings.start.rules.waterWithin),base=measure.sample(0);
assert(base.pumpTiles>0);assert.equal(base.plants.nearTreesAlive,r.analysis!.treesNear);assert.equal(base.plants.nearBushesAlive,r.analysis!.bushesNear);
...
const dead=a.plants.map(p=>p.dead);a.run({weather:'normal',days:1,cycle:6,occurrence:6,previous:'badtide'});
assert(dead.every((v,i)=>!v||a.plants[i].dead),'Dead plants do not revive during recovery');
assert(a.sim.D.every(x=>Number.isFinite(x)&&x>=0));assert(a.sim.C.every(x=>Number.isFinite(x)&&x>=0&&x<=1));
