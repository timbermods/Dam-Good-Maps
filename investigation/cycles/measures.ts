...
export interface Region {id:number;name:string;kind:string;x:number;y:number;tiles:number;initialVolume:number;}
export class Measures {
  ...
  constructor(readonly model:CycleModel, readonly waterWithin:number) {
    ...
    for(const basin of [true,false]) {
      ...
      for(let i=0;i<N;i++)if(c.labels[i]>=0)groups[c.labels[i]].push(i);
      for(const cells of groups) {
        const id=this.regions.length;
        let volume=0,x=0,y=0;
        for(const i of cells){this.labels[i]=id;volume+=b.water[i];x+=i%b.W;y+=Math.floor(i/b.W);}
        x=Math.round(x/cells.length);y=Math.round(y/cells.length);
    ...
    const blocked=new Uint8Array(N),links:[number,number][]=[];
    for(const o of b.entities) {
      if(WALK_BLOCKERS.has(o.template))for(const [x,y] of footprintTiles(o.template,o))if(x>=0&&y>=0&&x<b.W&&y<b.H)blocked[y*b.W+x]=1;
      if(o.template==='Slope'){
        const [dx,dy]=slopeHighSide(o.orientation),x=o.x+dx,y=o.y+dy;
        if(x>=0&&y>=0&&x<b.W&&y<b.H)links.push([o.y*b.W+o.x,y*b.W+x]);
      }
    }
    if(!b.start)throw new Error('Generated map has no start');
    ...
    for(let i=0;i<N;i++)if(flat[i]<=waterWithin&&b.heights[i]===b.start.z) {
      const x=i%b.W,y=Math.floor(i/b.W);
      for(const j of [y>0?i-b.W:-1,x>0?i-1:-1,y+1<b.H?i+b.W:-1,x+1<b.W?i+1:-1])if(j>=0)this.shore[j]=1;
    }
    this.firstDry=new Float32Array(N).fill(-1);this.firstBad=new Float32Array(N).fill(-1);
  }
  sample(day:number) {
    ...
    for(let i=0;i<N;i++) {
      const d=s.D[i],bad=s.C[i]>=BAD,lab=this.labels[i];total+=d;badVolume+=d*s.C[i];
      if(lab>=0){volume[lab]+=d;if(!bad)clean[lab]+=d;}else outside+=d;
      if(m.SC[i]>0)soil++;if(m.M[i]>0)moist++;
      if(wet[i]){
        wetTiles++;
        if(bad){badTiles++;if(this.firstBad[i]<0)this.firstBad[i]=day;}
        const parent=this.initialBodies[i];
        if(parent>=0){let set=descendants.get(parent);if(!set){set=new Set();descendants.set(parent,set);}set.add(c.labels[i]);}
      }
      if(this.initialWet[i]&&!wet[i]){dried++;if(this.firstDry[i]<0)this.firstDry[i]=day;}
      const surface=b.heights[i]+d;
      if(this.shore[i]&&d>=.3&&!bad&&surface>=b.start!.z-2&&surface<=b.start!.z+.01)pumpTiles++;
    }
    if(!pumpTiles&&this.firstWaterLost===null)this.firstWaterLost=day;
    ...
    m.plants.forEach((p,k)=>{
      ...
      if(!p.initialDead){if(tree)plants.originalTrees++;if(bush)plants.originalBushes++;if(p.dead){if(tree)plants.treesLost++;if(bush)plants.bushesLost++;}}
      if(this.nearPlants[k]){if(tree){plants.nearLogs+=SPECIES[p.species].logs;if(!p.dead)plants.nearTreesAlive++;}if(bush&&!p.dead)plants.nearBushesAlive++;}
    });
...
export function rle(values:ArrayLike<number>):number[] {
  const out:number[]=[];
  for(let i=0;i<values.length;){const v=values[i];let end=i+1;while(end<values.length&&values[end]===v)end++;out.push(end-i,v);i=end;}
  return out;
}
