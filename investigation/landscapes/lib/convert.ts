...
function bestStart(
  ...
  for (const c of candidates) {
    const dist = walkDistance(h, W, W, null, [], c, 24),
    ...
    const shore = shoreDistance(dist, h, W, W, mask, c.z),
      score =
...
export function convert(
  ...
  const selection = bestStart(h, water.depth, M, W, occupied),
  ...
  const measured = measureValidated(file, v, []),
    originalNatural = naturalness(
      original,
      W,
      W,
      null,
      [],
      Math.max(6000, 0.15 * N),
    );
  const fixture = {
    schema: 1,
    W,
  ...
  return {
    ...
    measured,
    originalNatural,
    features: featureVector(measured),
    selection: {
