...
- Significant simulated flow directions: outgoing edges above 0.005 and 15% of local outflow. Splits count wet cells with several outgoing edges; rejoins count cells receiving several edges, including thin receiving cells. Both divide by the count of flowing cells deeper than 0.05. The rejoin value is a ratio, not necessarily a bounded fraction. Enclosed dry components of four or more cells describe water loops.
...
Terrain targets include passing and failing conversions. Water-feature targets include only simulations that settled. Published fall statistics are null for unsettled runs: their terminal state is not a steady-state measurement. Other terminal-state water diagnostics remain in the records, flagged by `settled: false`, for investigating failures. Nulls stay null. The 22-level comparison is separate. These are exploratory bands from a convenience sample, not confidence intervals for world geography. Compare adjacent scales and both named and random cohorts before adopting a range.
