...
const water=canonicalSettle(modelFor(r.map));
...
assert.equal(water.depth[at(cut.cap)],0,'the old downstream arm must be cut off from through-flow');
...
passed.push('Maximum Wander forms a real narrow-neck bend, cuts a shortcut and leaves a canonically filled oxbow with a dry abandoned downstream arm');
passed.push('Oxbow carving stays monotone, conserves debris, creates no isolated extrema and replays/undoes exactly');
writeFileSync('captures/course-checks.json',JSON.stringify({passed,runs,endings,recoveryMoves:recoveries,maxTravelInMapWidths:maxStretch,headingLimitDegrees:110,progressWindowMoves:PROGRESS_WINDOW,oxbow:{step:cut.step,wetPoolStations:wetPool,settled:water.settled}},null,2)+'\n');
