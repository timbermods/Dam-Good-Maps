...
| Power location | Flow beside accessible land ↔ power away from the opening | `peakCleanFlux64`: largest sum of the four stored outflows at a clean wet tile beside safe dry ground within 64 walk units, m³/s. Also `waterfalls`, `geothermal64`, `nearestGeothermal`. | Flux is not velocity or hp. U01 is needed for legal wheel sites and output. A geothermal field needs construction; do not credit 400 operating hp at map start. |
...
| Resource timing | Large accessible stock ↔ a need to expand and renew | `logs20`: mature Pine/Birch/Oak log stock within 20 safe walk units, including `deadLogs20`. `readyBerries20`: mature living bushes with explicit ripe progress, goods. `scrap40`, `science64`, `mines64`. Also validator living trees and bushes. | Resource stock is not hourly delivery. Distances are from the start, not a placed work building; access and build/haul schedules need U03/U04/U07. |
...
- Block footprints use `footprintTiles`. The base analysis blocks Blockage rather than modelling its walkable top (U08). It does not model roofs, player buildings or newly built roads.
