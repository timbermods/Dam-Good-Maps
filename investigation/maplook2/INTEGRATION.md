...
The water shader uses the existing mesh's depth, badwater share and shore/fall flags. It adds depth absorption, transparent banks, a moving ripple normal, Schlick Fresnel against an original procedural sky, restrained sun glints and soft foam. Its reflection is an approximation of the sky, not a mirrored render of the terrain. No refraction render, screen-space reflection, texture download or game asset is needed. Keep the current overlay/hatching logic when adding this material to the editor; this demo intentionally draws only the clean comparison.
