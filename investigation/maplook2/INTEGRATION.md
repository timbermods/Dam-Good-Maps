...
The same texture carries contamination. Two small wet-neighbour smoothing passes and bilinear sampling create a multi-tile front while preserving pure clean/badwater regions. Do not diffuse across dry separators or different surface heights. Colour follows concentration through the measured mostly-clean warm tint, then murky brown; no stochastic red/blue mask. Retain the raw per-tile contamination for inspection and editing. Clean colour constants are unchanged; mixing colours are separately calibrated through the current sun/finish/blending. Recheck calibration if that pipeline changes. Waterfall curtains and foam formulas retain their accepted treatment. No refraction render, mirrored-scene reflection, texture download or game asset is needed. Keep existing overlays/hatching in the editor.
...
|---|---|---|
| Water | One transparent pass, two advected procedural-detail evaluations, a filtered flow/concentration lookup, flecks and blending. A 256² RGBA8 field costs 256 KiB. Field preparation happens on map load/change; no extra render target. Cost grows with visible water and overdraw. | Reduce fine noise work at distance; retain depth, concentration and flow direction. |
