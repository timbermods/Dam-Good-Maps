...
try {
  ...
  if(smoke){await page.screenshot({path:'captures/demo.jpg',type:'jpeg',quality:75});}
    ...
    for(let i=0;i<options.length;i++){
      ...
      if(i===0){await capture('river-128-badwater','badwater');await capture('river-128-badwater-top','badwater-top');await capture('river-128-badwater-low','badwater',0,0.55);await capture('river-128-soil','contaminated');await capture('river-128-ruins','ruins');}
