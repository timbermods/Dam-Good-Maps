...
try {
  ...
  const result=await page.evaluate(async()=>{
    ...
    const {surfaceVelocity}=await import('/flow.ts');
    const cardinal=[];
    ...
    const motions=[];
    for(const [name,x,y,axis] of [['lake',0,0,0],['slow east',1,0,0],['fast east',8,0,0],['fast west',-8,0,0],['fast north',0,8,1]]){
      flow(x,y);const before=pixels(8),after=pixels(8.1);let best={shift:0,error:Infinity};
    ...
    return {cardinal,motions,previous,phaseChanges:changes};
  });
  result.errors=errors;
  writeFileSync('captures/surface-check.json',JSON.stringify(result,null,2)+'\n');console.log(JSON.stringify(result,null,2));
  const [lake,slow,east,west,north]=result.motions;
