...
// "chrome"); nothing is published or served.
//
...
for (const size of sizes)
  for (const theme of themes) {
    const p: Row = { where: "node", gen: "proto", size, theme, ms: [], attempts: [] };
    const c: Row = { where: "node", gen: "current", size, theme, ms: [], attempts: [] };
    ...
    rows.push(p, c);
    console.log(`node ${size} ${theme}: proto median ${med(p.ms)} ms (max ${max(p.ms)}), current median ${med(c.ms)} ms (max ${max(c.ms)})`);
  }
  ...
  for (const size of sizes)
    for (const theme of themes)
      for (const gen of ["proto", "current"]) {
        const r: Row = { where: "chrome", gen, size, theme, ms: [], attempts: [] };
        for (const seed of seeds) {
        ...
        rows.push(r);
        console.log(`chrome ${size} ${theme} ${gen}: median ${med(r.ms)} ms (max ${max(r.ms)})`);
      }
  await browser.close();
}

const summary = rows.map((r) => ({ where: r.where, gen: r.gen, size: r.size, theme: r.theme, medianMs: med(r.ms), maxMs: max(r.ms), attempts: r.attempts }));
writeFileSync(join(process.cwd(), ".scratch", "bench.json"), JSON.stringify({ machine: `${cpus()[0]?.model} × ${cpus().length}`, node: process.version, seeds, summary }, null, 1));
