...
import { coordinatesForMinCorner, ORIENTATIONS, rotate, type Orientation } from "../../../src/core/format/footprints";
...
export function pickStart(
  ...
  const cands: { i: number; score: number; kind: string; o: Orientation; walk: number; levelled: boolean }[] = [];
  ...
  for (let y = margin; y < H - margin; y++) {
    for (let x = margin; x < W - margin; x++) {
      ...
      const score = pref * (0.5 + 0.5 * room) * (0.6 + 0.4 * Math.min(1, moistNear / 500)) + 0.25 * rng.float();
      cands.push({ i, score, kind, o, walk: w, levelled: uneven });
    }
  }
  ...
  const x = c.i % W;
  const y = (c.i - x) / W;
  return { x, y, level: h[c.i], orientation: c.o, kind: c.kind, shoreWalk: c.walk, levelled: c.levelled };
}
