...
export function planHydro(E: Float64Array, h: Uint8Array, g: Genome, seed: number, W: number, H: number, attempt: number): Hydro {
  const N = W * H;
  ...
  for (const tr of traced) {
    ...
    const cut = 1 + (big ? Math.round(g.hydro.incise) : 0);
    const floorHalf = big ? g.hydro.floor : g.hydro.floor * 0.3;
    const { st, prof, L, n } = profileOf(path, width, cut, true, rid);
