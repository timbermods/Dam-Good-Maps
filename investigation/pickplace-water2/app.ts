...
export function render(canvas:HTMLCanvasElement,h:Uint8Array,d:Float32Array,size:number,start:any,sources:any[]=[]) {
  ...
  for(const s of sources){ctx.fillStyle='#172a2a';ctx.fillRect(s.i%size-1,size-2-Math.floor(s.i/size),3,3);ctx.fillStyle='#fff';ctx.fillRect(s.i%size,size-1-Math.floor(s.i/size),1,1);}
}
...
$('form').addEventListener('submit',async e=>{
  ...
  $('download').hidden=true;$('result').hidden=true;$('prediction').hidden=true;$('suggestions').replaceChildren();
  ...
  try {
    ...
    render($('preview') as HTMLCanvasElement,r.heights,r.depth,s.size,s.start,s.waterDesign.sources);$('result').hidden=false;
    ...
    $('details').textContent+=`\nReal terrain, designed water. ${s.sourceCount} water origin${s.sourceCount===1?'':'s'}.\nGold: start. White: water origins.\nReturned area: ${s.lat.toFixed(5)}, ${s.lon.toFixed(5)} · ${s.size} × ${s.size}.\n${s.autoTry.attempts.length} water design${s.autoTry.attempts.length===1?'':'s'} tested. ${s.directPassed?'Original area passed.':'Automatic search tried other openings.'}`;
    const retained=s.reference.coverage>=.95&&s.drama.referenceWaterCells>=3?s.drama.retainedWaterFraction:null;
    $('details').textContent+=`\nSignature water: ${s.drama.signaturePresent?'present':'incomplete'}.\nObserved water retained: ${retained===null?'not measurable here':(retained*100).toFixed(1)+'%'}.\nReference coverage: ${(s.reference.coverage*100).toFixed(0)}%.`;
    if(!s.drama.signaturePresent)$('details').textContent+='\nThe main water feature is incomplete; this remains a preview.';
    if(s.passed) {downloadURL=URL.createObjectURL(new Blob([r.bytes],{type:'application/zip'}));const a=$('download') as HTMLAnchorElement;a.href=downloadURL;a.download=`Place-${s.lat}-${s.lon}-${s.size}.timber`;a.hidden=false;}
    if(!s.passed)for(const candidate of s.autoTry.suggestions.filter((c:any)=>c.reason==='different scale')) {
      const card=document.createElement('figure'),canvas=document.createElement('canvas'),caption=document.createElement('figcaption');
      render(canvas,Uint8Array.from(candidate.preview.heights),Float32Array.from(candidate.preview.depth),candidate.preview.size,null);
      caption.textContent=`${candidate.lat.toFixed(5)}, ${candidate.lon.toFixed(5)} · ${candidate.size} tiles · ${candidate.mpt} m/tile. Preview only: ${candidate.failures.map((c:any)=>c.message).join(' ')}`;
      card.append(canvas,caption);$('suggestions').append(card);
    }
