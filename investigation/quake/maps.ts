...
import { tree,startingLocation,waterSource,bush } from '../../src/core/format/entities';
...
export function fixture(kind='river',W=128):QuakeMap{
 ...
 for(let y=0;y<H;y++)for(let x=0;x<W;x++){
  ...
  if(kind==='river'){h=d<=3?6:d<=6?8:9;if(d<=3)depth[i]=.65;}
 ...
 if(kind==='river')for(let dx=-3;dx<=3;dx++)entities.push(waterSource({...at(rx+dx,H-1,'existing-source-'+dx),strength:.8}));
