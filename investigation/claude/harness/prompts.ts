...
- addResource {where, size}; moveStart {to: a place or [x, y]} (it plants berries and trees nearby when the new spot lacks them); moveFeature {target, by [dx, dy] | to: a tile, or for a set piece a place, where the app picks a checked site}; changeSetPiece {target, change: "wider", "a bit taller"}; changeFeature {target, set}; deleteFeature {target}; removeResources; sculpt; undoLast (alone: takes back your last accepted proposal).
