...
- addLake / addLandform / addResource {where, size}; addRiver {points from source to mouth, flow}; moveStart {to: a place or [x, y]} (it plants berries and trees nearby when the new spot lacks them); moveFeature {target, by [dx, dy] | to: a tile, or for a set piece a place, where the app picks a checked site}; changeSetPiece {target, change: "wider", "a bit taller"}; changeFeature {target, set}; resizeFeature {target, factor 0.5–2} (a drawn landform or lake, about its middle); deleteFeature {target}; removeResources; setRiverBadwater; sculpt; undoLast (alone: takes back your last accepted proposal).
- addSource {kind: water|badwater, at [x, y] or where, strength (blocks/s: water 0.25–8, badwater 0.25–72)}: where water starts, spreading at once; fillHollow: true puts a spring at the lowest point of the hollow there, which fills it into a lake. addRiver with live: true follows the editor's river tool: drawn from water it is a branch of it (no new source), and it may end on dry ground, where its water fills the hollow or runs on downhill; natural: true gives it gentle meanders.
- brush {tool: raise|lower|flatten|smooth|naturalize, where} paints the editor's own brushes over a place: amount 1–8 levels (raise, lower), level 0–16 (flatten; default the place's middle level), passes 1–8 (smooth, naturalize). Whole levels; its edge slopes a level a tile to the ground round it (a brush makes no cliffs), so a narrow place rises less than asked: the step's report says what moved and by how much. At most 30% of the map.
