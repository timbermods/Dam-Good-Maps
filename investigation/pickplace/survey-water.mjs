...
const places=JSON.parse(await readFile('places.json','utf8')).slice(Number(process.env.OFFSET||0),Number(process.env.OFFSET||0)+limit);
await mkdir('results-water/previews',{recursive:true});await mkdir('.work/maps-water',{recursive:true});
...
try {
  ...
  const rows=[];
  await writeFile('results-water/run.json',JSON.stringify({status:'running',date:new Date().toISOString(),places:places.length,sizes,attempts:0},null,2));
  for(const place of places) for(const size of sizes) {
    ...
    try {
      ...
      await writeFile(`results-water/previews/${id}.png`,PNG.sync.write(image));
      await writeFile(`.work/maps-water/${id}.timber`,Buffer.from(output.map,'base64'));
      await writeFile(`results-water/${id}.json`,compactJSON(row));
      console.log(`${id} ${place.name}: ${row.passed?'PASS':'FAIL'} ${(row.timings.totalMs/1000).toFixed(2)}s ${row.current.failures.map(c=>c.id).join(',')}`);
    } catch(e) {
      const row={...place,size,passed:false,error:String(e),totalMs:Date.now()-start};rows.push(row);await writeFile(`results-water/${id}.json`,JSON.stringify(row,null,2));console.log(`${id} ERROR ${e}`);
    }
  }
  await writeFile('results-water/run.json',JSON.stringify({date:new Date().toISOString(),browser:browser.version(),node:process.version,cpu:os.cpus()[0].model,logicalCpus:os.cpus().length,memoryGiB:os.totalmem()/2**30,base:'84b1866b4e1d444346949a7dd57dc0f3f9f69ac5',places:places.length,sizes,attempts:rows.length,passed:rows.filter(r=>r.passed).length,method:'Designed water; up to two source designs per area; up to seven scale/size/offset configurations; stop at first passing design; same-origin page creates module Worker; remote DEM fetched directly inside Worker; default browser security; ordinary browser Cache API shared across sizes.'},null,2));
