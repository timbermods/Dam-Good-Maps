...
const s=JSON.parse(await readFile('results/summary.json','utf8'));
const promising=s.prediction.find(p=>p.verdict==='promising');
const f=n=>n.toFixed(2);
const report=`# Pick a place
...
50 varied places × 96², 128² and 256²: **${s.passes}/150 (${(s.rate*100).toFixed(1)}%) passed the labelled local checks**. **${s.placesWithAPassingSize}/50 places** passed at one or more sizes. The unchanged base validator passed ${s.legacyPasses}/150. Every failure is retained. “Converts well” here means those checks pass; visual appeal and in-game play remain unverified.

| Size | Passes | Total median | Total p90 |
| --- | ---: | ---: | ---: |
${s.timing.map(r=>`| ${r.size}² | ${r.passes}/50 | ${f(r.medianSeconds)} s | ${f(r.p90Seconds)} s |`).join('\n')}

These are Edge worker measurements on one Ryzen 7 9800X3D machine, including fetching, analysis, simulation and export/reload. Later sizes reuse cached tiles. ${s.roundTripPasses}/150 exported files passed reload/load checks; ${s.errors} runtime errors. The page timer continued during every completed conversion. [Full results and failures](RESULTS.md), [all small renders](results/gallery.html), [sample maps](examples/README.md).
