...
$('form').addEventListener('submit',async e=>{
  ...
  try {
    const r=await convert({lat:coords[0],lon:coords[1],size:Number(($('size') as HTMLSelectElement).value),intention:($('intention') as HTMLSelectElement).value,...(scale==='auto'?{}:{mpt:Number(scale)})},s=>$('status').textContent=s,p=>{$('prediction').hidden=false;$('prediction').textContent=`Terrain preview: ${p.verdict}. ${p.warnings.join(' ')} ${p.suggestions.join(' ')}`;});
    ...
    if(!s.passed)for(const candidate of s.autoTry.suggestions.filter((c:any)=>c.reason==='nearby area')) {
