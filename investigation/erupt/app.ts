...
let pulse=0,pulseAt=0,morphAt=0;let personality=crypto.getRandomValues(new Uint32Array(1))[0];
...
function begin(intent:Intent,reroll=false){
  ...
  pendingBefore=view.capture();active=true;finishSent=false;animStart=null;queuedReady=false;ring.visible=arrow.visible=false;
  pulse=0;pulseAt=performance.now();
  if(!reroll){personality=(personality+1)>>>0;try{eruptionAnatomy=anatomy({W:view.W,H:view.H,heights:view.heights},settings(),intent);effects.set(eruptionAnatomy,view.heights,view.W);animStart=performance.now();}catch{}}
...
function undoEruption(){
  ...
  const entry=history.pop()!;redo.push(entry);view.restore(entry.before);restoreSaved();send({type:'undo'});notice('Eruption undone.');collect();
}
function redoEruption(){if(active||busy||!redo.length)return;const entry=redo.pop()!;history.push(entry);view.restore(entry.after);restoreSaved();send({type:'redo'});notice('Exact result restored.');collect();}
worker.onmessage=(event:MessageEvent)=>{
  ...
  switch(m.type){
    ...
    case 'started':active=true;showSettings(m.settings);lastSeed=m.settings.seed;$('seed-label').textContent='Personality '+lastSeed;
      eruptionAnatomy=m.anatomy;effects.set(m.anatomy,view.heights,view.W);settling=false;break;
...
function preview(){
  ...
  const origin=drag?.origin??hover,s=settings(),x=origin%view.W,y=Math.floor(origin/view.W),radius=naturalSize(s.power)*.5*(s.shape==='broad'?1.6:1)*(s.mode==='fissure'?.47:1),points:THREE.Vector3[]=[];
...
input('replay-file').onchange=async()=>{
  ...
  try{send({type:'replay',bundle:JSON.parse(await f.text())});}catch{notice('Could not read this eruption',true);}
};
...
function animate(t:number){
  ...
  if(queuedReady&&!batches.length){queuedReady=false;animStart??=t;effects.set(eruptionAnatomy!,pendingBefore!.heights,view.W);notice('The land is changing. Esc to revert.');}
  const age=animStart===null?2:(t-animStart)/1000;
  if(active&&animStart!==null&&!finishSent){

    view.progress=motion()?Math.min(1,(t-morphAt)/170):1;
    ...
    if(!busy&&!queuedReady&&!batches.length&&t-pulseAt>220){
      if(pulse<8){pulse++;pulseAt=t;send({type:'advance',progress:motion()?pulse/8:1});if(!motion())pulse=8;}
      else {finishSent=true;view.progress=1;send({type:'finish'});}
    }
  }
  ...
  effects.update(age,motion()&&active&&!settling,pulse/8);
